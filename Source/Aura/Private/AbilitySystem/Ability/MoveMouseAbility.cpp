...
UMoveMouseAbility::UMoveMouseAbility()
{
	...
	ActivationOwnedTags = FGameplayTagContainer();

	ActivationBlockedTags = FGameplayTagContainer(AuraGameplayTags::Character_State_HitReact);
	ActivationBlockedTags.AddTagFast(AuraGameplayTags::Character_State_Death);
...
void UMoveMouseAbility::CancelAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo,
	...
	if (AuraPlayerController->GetCursorHitResult().bBlockingHit)
	{
												...
												&AuraPlayerController->GetNavAgentPropertiesRef()))
		{
			UNavigationPath* NavPath = UNavigationSystemV1::FindPathToLocationSynchronously(this,
				AuraCharacter->GetActorLocation(), AuraPlayerController->AutoMoveDestination);
